...
export default class ParabolaDatabase {
  ...
  private async seed(filePath: string): Promise<QueryResult | undefined> {
    try {
  ...
  private async seedFile(filename: string): Promise<QueryResult> {
    try {
  ...
  // async #clean(): Promise<void> { // typescript private method
  ...
  // this is what establishes our connection to neo4j each time
  private getConnection(databaseName = this.databaseName): Neo4jConnection {
