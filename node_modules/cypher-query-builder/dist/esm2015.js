...
import neo4j__default, { isInt, integer, driver, auth } from 'neo4j-driver';
...
class TermListClause extends Clause {
  ...
  stringifyProperties(props, alias, node) {
    ...
    return _reduce(_castArray(props), convertToString, []);
  }
...
class Skip extends Clause {
  constructor(amount) {
    super();
    this.amount = amount;
    this.amountParam = this.addParam(neo4j__default.int(amount), 'skipCount');
  }
...
class Limit extends Clause {
  constructor(amount) {
    super();
    this.amount = amount;
    this.amountParam = this.addParam(neo4j__default.int(amount), 'limitCount');
  }
...
class Transformer {
  ...
  transformNode(node) {
    return {
      identity: integer.toString(node.identity),
      labels: node.labels,
  ...
  transformRelation(rel) {
    return {
      identity: integer.toString(rel.identity),
      start: integer.toString(rel.start),
      end: integer.toString(rel.end),
      label: rel.type,
  ...
  convertInteger(num) {
    if (integer.inSafeRange(num)) {
      return integer.toNumber(num);
    }
...
class Connection extends Builder {
   ...
   */
  constructor(url, auth$1, options = driver) {
    super();
    this.url = url;
    this.transformer = new Transformer();
    this.auth = isCredentials(auth$1) ? auth.basic(auth$1.username, auth$1.password) : auth$1;
    const driverConstructor = isTrueFunction(options) ? options : options.driverConstructor ? options.driverConstructor : driver;
  ...
  run(query) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      return session.run(queryObj.query, queryObj.params).then(({
        ...
        yield session.close();
        return this.transformer.transformRecords(records);
  ...
  stream(query) {
    return new Observable(subscriber => {
      ...
      result.subscribe({
        ...
        onCompleted: () => __awaiter(this, void 0, void 0, function* () {
          yield session.close();
